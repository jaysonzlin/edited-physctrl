...
class SpatialTemporalTransformer(ModelMixin, ConfigMixin, PeftAdapterMixin):
    """
    ...
    def __init__(
        ...
        self.transformer_block = "SpatialTemporalTransformerBlock"
...
class MCG_DIT(nn.Module):
    def __init__(self, n_points, n_frame, n_feats, model_config):
        ...
        self.dit = SpatialTemporalTransformer(sample_points=n_points, sample_frames=n_frame+self.cond_frame, in_channels=n_feats,
    ...
    def forward(self, x, timesteps, init_pc, v1, v2, w1, w2, y=None, null_emb=None):
        ...
        v1 = v1.unsqueeze(1) if v1.ndim == 2 else v1
...
if __name__ == "__main__":
    ...
    point_num = 4096
    frame_num = 24
    x = torch.randn(1, frame_num, point_num, 3).to(device).to(torch.float32)
    timesteps = torch.tensor([999]).int().to(device).to(torch.float32)
    init_pc = torch.randn(1, point_num, 3).to(device).to(torch.float32)
    v1 = torch.randn(1, 3).to(device).to(torch.float32)
    ...
    model = MCG_DIT(point_num, frame_num, 3, cfg.model_config).to(device).to(torch.float32)
    model.train()
